...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!reps || reps.length === 0) {
      return new Response(
        JSON.stringify({ success: true, message: "No verified representatives found", emailsSent: 0, companiesContacted: 0 }),
    ...
    if (targetReps.length === 0) {
      return new Response(
        JSON.stringify({ success: true, message: "No eligible representatives found", emailsSent: 0, companiesContacted: 0 }),
        { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } }
      );
    }
    ...
    for (const rep of targetReps as any[]) {
          ...
          <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: #f9fafb; padding: 32px 30px; border-radius: 0 0 10px 10px; border: 1px solid #e5e7eb; border-top: none;">
              ...
              <p style="font-size: 15px; color: #374151;">
                We are reaching out regarding the <strong>DLinRT.eu Company Certification Program</strong> for <strong>${companyName}</strong>.
              </p>
      ...
      try {
        const emailResponse = await resend.emails.send({
          ...
          to: [email],
          cc: [INFO_EMAIL],
          subject: `Action Required: Certify Your Product Information on DLinRT.eu – ${companyName}`,
          html: htmlContent,
        });
    ...
    const summary = {
      ...
      companiesList: Array.from(companiesContacted),
    };
