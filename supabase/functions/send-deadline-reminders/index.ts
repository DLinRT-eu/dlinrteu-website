...
const getUrgencyLevel = (daysUntil: number): { label: string; color: string; bgColor: string } => {
  if (daysUntil < 0) return { label: 'OVERDUE', color: '#dc2626', bgColor: '#fef2f2' };
  if (daysUntil === 0) return { label: 'DUE TODAY', color: '#dc2626', bgColor: '#fef2f2' };
  if (daysUntil === 1) return { label: 'DUE TOMORROW', color: '#ea580c', bgColor: '#fff7ed' };
...
const handler = async (req: Request): Promise<Response> => {
  ...
  if (!isServiceRole) {
    ...
    const adminClient = createAuthClient(supabaseUrl, serviceRoleKey!);
  ...
  try {
    ...
    if (!enabled && !forceRun) {
      ...
      return new Response(JSON.stringify({ success: true, message: "Reminders are disabled", sent: 0 }), {
    ...
    if (!reviews || reviews.length === 0) {
      ...
      return new Response(JSON.stringify({ success: true, message: "No reminders needed", sent: 0, settings: { threshold_days: finalThreshold, min_interval_hours: finalInterval } }), {
        status: 200, headers: { "Content-Type": "application/json", ...corsHeaders },
      });
    }
    ...
    // Fetch notification preferences for all reviewers
    const reviewerIds = Array.from(reviewsByReviewer.keys());
    ...
    for (const [reviewerId, reviewerReviews] of reviewsByReviewer.entries()) {
      // Check notification preferences (review_deadlines category)
      const prefs = (prefsMap.get(reviewerId) as any) || {};
      const categoryPrefs = prefs?.categories?.review_deadlines;
      if (categoryPrefs?.email === false) {
        console.log(`Reviewer ${reviewerId} has disabled email for review_deadlines, skipping`);
        emailsSkipped++;
        continue;
      }

      const reviewer = reviewerReviews[0];
      ...
      const reviewListHtml = reviewerReviews.map(review => {
        ...
        const deadlineDate = new Date(review.deadline).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
        return `
          ...
          <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e5e7eb; border-top: none;">
              ...
              <div style="display: flex; gap: 10px; margin: 20px 0; flex-wrap: wrap;">
                ...
                ${dueTodayCount > 0 ? `<div style="background: #fff7ed; padding: 12px 20px; border-radius: 8px; text-align: center; flex: 1; min-width: 100px;"><div style="font-size: 24px; font-weight: bold; color: #ea580c;">${dueTodayCount}</div><div style="font-size: 12px; color: #c2410c;">Due Today</div></div>` : ''}
                ${dueSoonCount > 0 ? `<div style="background: #fffbeb; padding: 12px 20px; border-radius: 8px; text-align: center; flex: 1; min-width: 100px;"><div style="font-size: 24px; font-weight: bold; color: #d97706;">${dueSoonCount}</div><div style="font-size: 12px; color: #b45309;">Due Soon</div></div>` : ''}
              </div>
              ...
              <div style="text-align: center; margin: 30px 0;">
                <a href="${siteUrl}/reviewer/dashboard" style="display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 14px 32px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px;">Go to Reviewer Dashboard</a>
              </div>
              <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; color: #6b7280; font-size: 14px;">
                <p style="margin: 5px 0;"><strong>Need help?</strong> Contact your administrator if you have questions about your assignments.</p>
                <p style="margin: 5px 0; color: #9ca3af;">This is an automated reminder from DLinRT.eu. You will receive at most one reminder per day.</p>
              </div>
            </div>
          </body>
        </html>
