...
class DataService {
  ...
  /**
   * Get all companies that have at least one product (active or pipeline).
   * This is the single source of truth for company counts across the platform.
   */
  getAllCompaniesWithProducts(): CompanyDetails[] {
    return COMPANIES.filter(company =>
      ALL_PRODUCTS.some(product => company.productIds.includes(product.id || ''))
    );
  }
