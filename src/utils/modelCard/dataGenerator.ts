  ...
  return {
    ...
    performance: {
      ...
      evidenceLevel: product.evidenceRigor
        ? `${product.evidenceRigor}/${product.clinicalImpact || 'N/A'}${product.adoptionReadiness ? `/${product.adoptionReadiness}` : ''}`
        : "N/A",
      evidenceLevelNotes: product.evidenceRigorNotes || product.clinicalImpactNotes || "N/A",
      dosePredictionModels: doseModelsText,
    },
    evidenceScore: (() => {
      const summary = buildProductEvidenceSummary(product);
      return {
        evidenceRigor: summary["Evidence Rigor"],
        evidenceRigorNotes: summary["Evidence Rigor Notes"],
        clinicalImpact: summary["Clinical Impact"],
        clinicalImpactNotes: summary["Clinical Impact Notes"],
        adoptionReadiness: summary["Adoption Readiness"],
        adoptionReadinessNotes: summary["Adoption Readiness Notes"],
        readinessSignal: summary["Readiness Signal"],
        scoreOrigin: summary["Score Origin"],
        scoreOverrideReason: summary["Score Override Reason"],
        scoredPublicationsCount: summary["Scored Publications Count"],
        listedPublicationsCount: summary["Listed Publications Count"],
      };
    })(),
    evidenceSources: buildEvidenceBySourceRows([product]).map((row) => ({
      title: row.Title,
      authors: row.Authors,
      journal: row.Journal,
      year: row.Year,
      ...
      pmid: row.PMID,
      link: row.Link,
      evidenceRigor: row["Evidence Rigor"],
      clinicalImpact: row["Clinical Impact"],
      rationale: row.Rationale,
      appliesToCategory: row["Applies To Category"],
      vendorIndependent: row["Vendor Independent"],
      multiCenter: row["Multi-Center"],
      multiNational: row["Multi-National"],
      prospective: row.Prospective,
      externalValidation: row["External Validation"],
      setsProductRigor: row["Sets Product Rigor"],
      setsProductImpact: row["Sets Product Impact"],
      note: row.Note,
    })),
    studyQuality: {
