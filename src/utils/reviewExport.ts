...
import { createExcelWorkbook, type ExcelSheet } from "@/utils/excelExport";
...
export const exportReviewToCSV = (
  ...
  const headers = [
    'Product ID', 'Product Name', 'Company', 'Category', 'Status', 'Urgency',
    'Days Since Review', 'Critical Issues', 'Warnings', 'Total Issues',
    'Last Revised', 'Assignee', 'Notes', 'Issue Details'
  ];
  
  const csvData = exportData.map(row => [
    row.productId, row.productName, row.company, row.category, row.status,
    row.urgency, row.daysSinceReview.toString(), row.criticalIssues.toString(),
    row.warnings.toString(), row.totalIssues.toString(), row.lastRevised,
    row.assignee, row.notes, row.issueDetails
  ]);
...
export const exportReviewToExcel = async (
  products: ReviewProduct[],
  assignments: Record<string, string> = {}
) => {
  const exportData = generateReviewExportData(products, assignments);
  ...
  a.setAttribute('hidden', '');
  a.setAttribute('href', url);
  a.setAttribute('download', `product_review_export_${new Date().toISOString().split('T')[0]}.xlsx`);
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  window.URL.revokeObjectURL(url);
