...
export const THERAPANACEA_ADAPTBOX_PRODUCTS: ProductDetails[] = [
  {
    ...
    limitations: [
      "Single AI auto-contouring model is Pelvis (Male); H&N and Breast/Thorax are FDA-cleared sCT-generation anatomies only — not auto-contouring models",
      "CBCT-based synthetic CT: augmented slices outside the native CBCT field of view are generated by duplicating planning-CT slices after rigid registration and are NOT reliable for contour propagation or dose calculation",
      "Bad CBCT image quality (artefacts, motion, uncommonly high slice thickness, lossy compression) can lead to incorrect synthetic CT or dose — the user must verify the synCT before any clinical action",
      "Synthetic CT, contours and dose generated by AdaptBox must be reviewed and validated by an authorized user before clinical replanning decisions",
      "User must ensure the RTPlan and RTStruct submitted for an AdaptBox analysis correspond to the ones used for patient treatment",
      "Adult patients only (ART-Plan+ is not intended for patients <18 years)",
      "When several RTPlans are linked to the same RTSS, only the latest is used",
